...
- 실시간 조회 → 비동기 재시도 → 배치 대사로 이어지는 3계층 정합성 시스템 설계 → **거래 일관성 약 100% 달성**
...
2. **애플리케이션 레벨 (Optimistic Locking)**
   ...
   - 위치: `BatchTargetEntity.kt:189-191`
...
3. **비즈니스 레벨 (멱등성 보장)**
   ...
   - 위치: `ReconciliationWorker.kt:199-205`

**결과:**
- ✅ 중복 처리 완전 방지: Unique Constraint + Version Lock으로 데이터 정합성 100% 보장
...
위치: `ReconciliationJob.kt:364-374`
