...
월 200 ~ 250만 건 규모의 결제 시스템 구축에 참여했으며, 특히 실시간 대사 시스템을 2.5년간 전담하여 전체 코드의 84%를 기여했습니다. 3계층 정합성 검증 시스템을 통해 높은 수준의 거래 일관성을 지향하며 일 6 ~ 7만 건의 자동 대사를 처리하도록 시스템을 설계했습니다. 검증 프로세스를 개선하여 소요 시간을 83% 단축했고(30 ~ 40분 → 5분), PG API 호출을 50 ~ 70% 감소시켜 배치 처리 시간을 30 ~ 50% 단축했습니다.
...
- 실시간 조회 → 비동기 재시도 → 배치 대사로 이어지는 3계층 정합성 시스템 설계 → **높은 수준의 거래 일관성 달성**
...
// 캐시 히트 추적
if (pgInquiryCache.size == cacheSizeBefore) {
...
**안전성 보장**
- Worker 격리된 메모리 공간 → 동시성 이슈 방지
- Step 종료 시 자동 소멸 → Stale Data 우려 없음
- **Unknown/Failed는 캐싱 안 함** (재시도 필요)

**결과**
- PG API 호출 50 ~ 70% 감소 (10,000회/일 → 3,000 ~ 5,000회/일)
- 배치 처리 시간 30 ~ 50% 단축 (60분 → 30 ~ 42분)
- PG 서버 부하 감소 → API Rate Limit 여유 확보
...
- PG 중복 조회 제거: 멱등성 설계로 재시도 시에도 안전
...
- **순서 보장 유지** → 동일 날짜 + 동일 번호는 항상 같은 파티션 (멱등성 보장)
...
3계층 독립 검증 시스템을 통해 높은 수준의 거래 상태 조회 신뢰성을 유지하며 일일 6 ~ 7만 건의 자동 대사를 처리하고 있으며, 월 600건 이상의 승인취소 실패 거래를 자동으로 재시도합니다. 이를 통해 CS 운영 비용을 약 3% 절감했습니다.
